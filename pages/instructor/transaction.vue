      ...
      <template #default>
        ...
        <table v-else class="table table-pin-rows">
          ...
          <tbody>
            ...
            >
              <tr class="hover cursor-pointer"  @click="showTransactionModal(transaction)">
                <th>{{ index + 1 }}</th>
                <td>
                ...
                <TransactionModal :transaction="transaction" @approve-borrow-req="approveBorrowReq" @approve-return-req="approveReturnReq"/>
              </tr>
            </template>
          </tbody>
        </table>
...
const filteredTransactions = computed(() => {
  if (selectedStatus.value === 'すべて') {
    ...
    const filteredTransactions = allTransactions.value.filter((transaction) => transaction.status === selectedStatus.value);
...
const showTransactionModal = (transaction:Transaction) => {
    ...
    if(transaction.status === "貸出確認" || transaction.status === "返却確認") {
       document.getElementById(`transaction-${transaction.transactionid}`)?.showModal(); 
    }
};
